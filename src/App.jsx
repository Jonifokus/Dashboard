...
const VIS = [
  { key: 'VALID', label: 'Valid', color: P.valid },
  { key: 'OBSERVE', label: 'Observe', color: P.observe },
  { key: 'INVESTIGATE', label: 'Investigate', color: P.investigate },
  { key: 'INCOMPLETE', label: 'Incomplete', color: P.incomplete },
];
...
function getRegionCode(cl) {
  if (!cl) return '??';
  const m = cl.match(/^([A-Z]{2,3})[- _]/);
  return m ? m[1] : cl.slice(0, 3).toUpperCase();
}
...
function readFileRows(buf) {
  ...
  const asLastC = hdrMap['Activity Status']
    ? hdrMap['Activity Status'].slice(-1)[0]
    : findCol('Activity Status');
...
function processRows(rows) {
  ...
  rows.forEach((r) => {
    ...
    if (!['VALID', 'OBSERVE', 'INVESTIGATE', 'INCOMPLETE'].includes(vs)) {
      ...
      if (rawAS.startsWith('A1')) vs = 'VALID';
        ...
        const hasIn =
          r['Check-In Latitude'] != null && r['Check-In Longitude'] != null;
        const hasOut =
          r['Check-Out Latitude'] != null && r['Check-Out Longitude'] != null;
        if (!hasIn || !hasOut) vs = 'INCOMPLETE';
        else {
          const mx = Math.max(
            parseFloat(r['Distance Check In (Meter)']) || 0,
            parseFloat(r['Distance Check Out (Meter)']) || 0
          );
    ...
    if (!['NORMAL', 'SHORT', 'LONG'].includes(dur)) {
      ...
      if (isNaN(dm)) {
        const tIn = r['Actual Visit Time']
          ? new Date(r['Actual Visit Time'])
          : null;
        const tOut = r['Actual Check-Out Time']
          ? new Date(r['Actual Check-Out Time'])
          : null;
    ...
    if (!['MATCH', 'NOT MATCH', 'INCOMPLETE'].includes(loc.toUpperCase())) {
      ...
      if (!hasIn) loc = 'INCOMPLETE';
        ...
        const dlat = Math.abs(
          parseFloat(r['Check-In Latitude']) - parseFloat(r['RO Latitude'])
        );
        const dlng = Math.abs(
          parseFloat(r['Check-In Longitude']) - parseFloat(r['RO Longitude'])
        );
        // ~0.005 deg ≈ 500m
    ...
    // In Range
    const inR = String(r['In Range'] || '')
      .trim()
      .toUpperCase();
    const inRKey =
      inR === 'YES' || inR === 'Y' || inR === '1' || inR === 'TRUE'
        ? 'YES'
        : 'NO';
    ...
    // Census tracking
    const censusVal = String(r['RO Census'] || '')
      .trim()
      .toUpperCase();
    const isCensus =
      censusVal === 'Y' ||
    ...
    if (!isNaN(durM)) {
      canvMap[nm].durSum += durM;
      canvMap[nm].durCnt++;
    }
    if (!isNaN(disM)) {
      canvMap[nm].disSum += disM;
      canvMap[nm].disCnt++;
    }
    // In Range per canvasser
    if (r['In Range'] != null) {
      if (inRKey === 'YES') canvMap[nm].IR_YES++;
      else canvMap[nm].IR_NO++;
    }
    ...
    if (dt) {
      if (!dateMap[dt])
        dateMap[dt] = { date: dt, total: 0, A1: 0, A2: 0, A3: 0 };
      dateMap[dt].total++;
  ...
  const canvassers = Object.values(canvMap).map((c) => ({
    ...c,
    avgDur: c.durCnt ? +(c.durSum / c.durCnt).toFixed(1) : null,
    avgDis: c.disCnt ? +(c.disSum / c.disCnt).toFixed(1) : null,
    a1p: pct(c.A1, c.total),
    a2p: pct(c.A2, c.total),
    a3p: pct(c.A3, c.total),
    invP: pct(c.INVESTIGATE, c.total),
  }));

  const censusData = Object.values(outMap)
    .filter((d) => d._isCensus)
  ...
  return {
    ...
    locC,
    inRangeC,
    outletData: Object.values(outMap)
      .filter((d) => !d._isCensus)
      .sort((a, b) => b.total - a.total),
    censusData,
    canvassers,
    trend: Object.values(dateMap).sort((a, b) => a.date.localeCompare(b.date)),
    duplicates: Object.values(visitMap)
      .filter((v) => Array.isArray(v.visits) && v.visits.length > 1)
...
function aggregateList(dataList) {
  ...
  dataList.forEach((r) =>
    (r.trend || []).forEach((d) => {
      if (!tMap[d.date])
        tMap[d.date] = { date: d.date, total: 0, A1: 0, A2: 0, A3: 0 };
      ['total', 'A1', 'A2', 'A3'].forEach((f) => {
        tMap[d.date][f] = (tMap[d.date][f] || 0) + (d[f] || 0);
      });
    })
  );
  const CANV_KEYS = [
  ...
  dataList.forEach((r) =>
    (r.canvassers || []).forEach((c) => {
      if (!cMap[c.name]) cMap[c.name] = { ...c };
        ...
        CANV_KEYS.forEach((k) => {
          cMap[c.name][k] = (cMap[c.name][k] || 0) + (c[k] || 0);
        });
      }
    })
  );
  const canvassers = Object.values(cMap).map((c) => ({
    ...c,
    avgDur: c.durCnt ? +(c.durSum / c.durCnt).toFixed(1) : null,
    avgDis: c.disCnt ? +(c.disSum / c.disCnt).toFixed(1) : null,
    a1p: pct(c.A1, c.total),
...
function Tip({ active, payload, label, t }) {
  ...
  return (
    ...
    >
      <p style={{ fontWeight: 700, marginBottom: 6, color: P.accent }}>
        {label}
      </p>
      {payload.map((p, i) => (
    ...
    {[
      ...
      (s, i) =>
        s.v > 0 && (
          <div
            key={i}
            style={{ width: pct(s.v, total) + '%', background: s.c }}
          />
        )
    )}
  </div>
...
function Pagination({ page, setPage, total, pageSize, t }) {
  ...
  const s = page * pageSize + 1,
    e = Math.min((page + 1) * pageSize, total);
  const ps = Array.from({ length: tp }, (_, i) => i).filter(
    (i) => Math.abs(i - page) <= 2
  );
  const btn = (a, d) => ({
    background: a ? P.accent : d ? 'transparent' : t.cardAlt,
    color: a ? '#fff' : d ? t.muted : t.text,
    border: `1px solid ${a ? 'transparent' : t.border}`,
    borderRadius: 6,
    padding: '5px 10px',
    fontSize: 11,
    fontWeight: 700,
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', gap: 4 }}>
        ...
        <button
          onClick={() => setPage((p) => Math.min(tp - 1, p + 1))}
          disabled={page >= tp - 1}
          style={btn(false, page >= tp - 1)}
        >
          ›
        </button>
        <button
          onClick={() => setPage(tp - 1)}
          disabled={page >= tp - 1}
          style={btn(false, page >= tp - 1)}
        >
          »
...
function DrillDownPanel({ drill, onClose, t, onCanvasserClick }) {
  ...
  const filt = drill.rows.filter((r) =>
    search
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 11, color: t.muted, marginTop: 1 }}>
              {drill.rows.length} canvasser · {drill.total.toLocaleString()}{' '}
              aktivitas
            </div>
          </div>
          <button
        ...
        >
          <input
            placeholder="🔍 Cari canvasser / cluster..."
            value={search}
            onChange={(e) => {
          ...
          <div style={{ fontSize: 10, color: t.muted, marginTop: 4 }}>
            💡 Klik nama untuk lihat detail aktivitas
          </div>
        </div>
            ...
            <tbody>
              {list.map((r, i) => (
                <tr
                  ...
                  onMouseEnter={(e) =>
                    (e.currentTarget.style.background = drill.color + '18')
                  }
                  onMouseLeave={(e) =>
                ...
                >
                  ...
                  <td style={{ padding: '8px 10px' }}>
                    ...
                    >
                        ...
                        <div
                          style={{
                            width:
                              Math.min(
                                100,
                                pct(r.count, drill.rows[0]?.count || 1)
                              ) + '%',
                            height: '100%',
                            borderRadius: 3,
                            background: drill.color,
                      ...
                      <span style={{ fontWeight: 700, color: drill.color }}>
                  ...
                  >
                    {pctS(r.count, r.total)}
                  </td>
                  <td style={{ padding: '8px 10px' }}>
                    <button
                      style={{
                        background: drill.color + '20',
                        border: 'none',
                        color: drill.color,
                        borderRadius: 6,
                        padding: '3px 8px',
...
function CanvasserDetailPanel({ detail, onClose, t }) {
  ...
  const fmtDur = (v) => {
    const n = parseFloat(v);
    if (isNaN(n)) return '–';
    if (n >= 60) return (n / 60).toFixed(1) + 'j';
    if (n >= 1) return n.toFixed(1) + 'mnt';
    return Math.round(n * 60) + 'det';
  };
  const vsColor = (vs) => {
    const u = String(vs || '').toUpperCase();
    return u === 'VALID'
      ? P.valid
      : u === 'OBSERVE'
      ? P.observe
  ...
  const reason = (r) => {
    ...
    if (durSt === 'SHORT' || (!isNaN(dur) && dur > 0 && dur < 3))
      f.push(`⏱ Durasi singkat (${fmtDur(dur)})`);
    else if (durSt === 'LONG' || (!isNaN(dur) && dur > 60))
      f.push(`⏱ Durasi panjang (${fmtDur(dur)})`);
    // Distance
    if (distIn > 5000)
      f.push(`🚨 Check-in sangat jauh dari outlet (${fmtDist(distIn)})`);
    else if (distOut > 5000)
      f.push(`🚨 Check-out sangat jauh dari outlet (${fmtDist(distOut)})`);
    else if (distIn > 500)
      f.push(`📍 Check-in jauh dari outlet (${fmtDist(distIn)})`);
    else if (distOut > 500)
      f.push(`📍 Check-out jauh dari outlet (${fmtDist(distOut)})`);
    // Location
    ...
    >
      ...
      >
          ...
          <div style={{ flex: 1 }}>
            ...
            >
              ...
              <span style={{ color: t.muted }}>
                · {sorted.length} aktivitas sesuai filter
              </span>
            </div>
          </div>
          <button
        ...
        <div style={{ overflowY: 'auto', flex: 1 }}>
                    ...
                    <td style={{ padding: '7px 10px' }}>
                      {r['In Range'] != null ? (
                        <span
                          style={{
                            background:
                              String(r['In Range']).toLowerCase() === 'yes' ||
                              String(r['In Range']) === '1'
                                ? P.a1 + '22'
                                : P.investigate + '22',
                            color:
                      ...
                      style={{
                        ...
                        color:
                          dist > 500
                            ...
                            ? P.observe
                            : t.muted,
                        fontWeight: dist > 500 ? 700 : 400,
                      }}
                    >
                      {fmtDist(r['Distance Check In (Meter)'])}
                    </td>
                    <td
                      style={{
                        padding: '7px 10px',
                        color: dur > 0 && dur < 3 ? P.short : t.muted,
                        fontWeight: dur > 0 && dur < 3 ? 700 : 400,
                      }}
                    >
          ...
          >
            <b>* Jarak ke Outlet</b> = selisih koordinat GPS HP canvasser vs
            koordinat outlet terdaftar saat check-in/out. Bukan jarak perjalanan
            — jarak besar berarti canvasser kemungkinan tidak berada di lokasi
            outlet.
          </div>
        </div>
      </div>
...
function UploadScreen({ onLoad, t }) {
  ...
  const handleFiles = useCallback(async (files) => {
    ...
    try {
      ...
      if (!xlsFiles.length)
        throw new Error('Tidak ada file .xlsx/.xls ditemukan');
            ...
            new Promise((res, rej) => {
              ...
              reader.onload = (e) => {
                try {
                  ...
                  if (!rows.length)
                    throw new Error(`${f.name}: tidak ada data`);
      ...
      setQueue((prev) => {
        ...
        results.forEach((r) => {
          ...
          const byLabel = m.findIndex(
            (x) => x.label === r.label && x.name !== r.name
          );
          if (byName >= 0) {
        ...
        if (skipped.length > 0) {
          setError(
            `⚠️ Cluster berikut sudah ada, dilewati: ${skipped.join(', ')}`
          );
  ...
  const regionGroups = {};
  queue.forEach((f) => {
    if (!regionGroups[f.regionCode]) regionGroups[f.regionCode] = [];
    regionGroups[f.regionCode].push(f);
  });
    ...
    >
      <div style={{ textAlign: 'center', marginBottom: 32 }}>
        ...
        >
          <div
            style={{
              ...
              height: 42,
              borderRadius: 10,
              background: 'linear-gradient(135deg,#1d5fc0,#2d8ef5)',
              display: 'flex',
              alignItems: 'center',
              justifyContent: 'center',
              fontWeight: 900,
          ...
          <div>
            <div style={{ fontSize: 20, fontWeight: 800, color: t.text }}>
              XLSMART <span style={{ color: '#3b82f6' }}>Analytics</span>
            </div>
            <div
              style={{
                fontSize: 11,
        ...
        <p style={{ color: t.muted, fontSize: 13, margin: 0 }}>
          Upload file per cluster — dashboard otomatis kelompokkan per region &
          nasional
        </p>
      </div>
      ...
      <div
        ...
        style={{
          width: '100%',
          maxWidth: 520,
          border: `2px dashed ${drag ? P.accent : t.border}`,
          borderRadius: 20,
          padding: '36px 28px',
          textAlign: 'center',
          background: drag ? 'rgba(37,99,235,0.06)' : t.card,
          transition: 'all 0.2s',
        }}
      >
        <input
        ...
        {loading ? (
          ...
          <>
            ...
            >
              {drag ? 'Lepas di sini!' : 'Drag & drop file XLS/XLSX'}
            </div>
            <div style={{ color: t.muted, fontSize: 12, marginBottom: 18 }}>
              Atau pilih file / folder
            </div>
            <div
              style={{
                display: 'flex',
      ...
      {error && (
        <div
          style={{
            marginTop: 12,
            background: error.startsWith('⚠️')
              ? 'rgba(245,158,11,0.1)'
              : 'rgba(239,68,68,0.1)',
            border: `1px solid ${
              error.startsWith('⚠️') ? '#f59e0b' : '#ef4444'
            }`,
            borderRadius: 10,
            padding: '10px 18px',
            color: error.startsWith('⚠️') ? '#fbbf24' : '#f87171',
            fontSize: 12,
            maxWidth: 520,
          }}
        >
        ...
        <div style={{ width: '100%', maxWidth: 520, marginTop: 18 }}>
          ...
          {Object.entries(regionGroups).map(([rgn, files], ri) => (
            <div key={rgn} style={{ marginBottom: 14 }}>
              ...
              >
                <span
                  style={{
                    background: P.regions[ri % P.regions.length] + '22',
                    color: P.regions[ri % P.regions.length],
                    padding: '2px 10px',
                    borderRadius: 999,
                    fontWeight: 800,
                  }}
                ...
                >
                  <div
                    style={{
                      ...
                      height: 8,
                      borderRadius: 2,
                      background: P.regions[ri % P.regions.length],
                      flexShrink: 0,
                    }}
                  />
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 11, color: t.muted }}>
                      {f.rows.length.toLocaleString()} aktivitas
                    </div>
                  </div>
                  <button
                    onClick={() =>
          ...
          >
            <div style={{ fontSize: 11, color: t.muted, flex: 1 }}>
              {queue.length} cluster · {Object.keys(regionGroups).length} region
              · {queue.reduce((s, f) => s + f.rows.length, 0).toLocaleString()}{' '}
              aktivitas
            </div>
            <button
              onClick={() => onLoad(queue)}
              style={{
                background: 'linear-gradient(135deg,#1d5fc0,#2d8ef5)',
                color: '#fff',
                border: 'none',
...
function Dashboard({ files, onReset, dark, toggleDark }) {
  ...
  const [canvDetail, setCanvDetail] = useState(null); // {canvasser, drillLabel, color}
  // Responsive
  ...
  const clusters = useMemo(
    () =>
      files.map((f, i) => ({
        ...
        color: P.regions[i % P.regions.length],
        fileName: f.name,
      })),
    [files]
  );
  ...
  const regionGroups = useMemo(() => {
    const g = {};
    clusters.forEach((c) => {
      if (!g[c.regionCode]) g[c.regionCode] = [];
      g[c.regionCode].push(c);
    });
    return g;
  }, [clusters]);

  const regionCodes = useMemo(
    () => Object.keys(regionGroups).sort(),
    [regionGroups]
  );
  ...
  const regionAgg = useMemo(() => {
    ...
    regionCodes.forEach((code, i) => {
      r[code] = {
        ...aggregateList(regionGroups[code]),
        label: code,
        regionCode: code,
        color: P.regions[i % P.regions.length],
      };
  ...
  const view = useMemo(() => {
    if (selCluster)
      return clusters.find((c) => c.label === selCluster) || national;
  ...
  const compData = useMemo(() => {
    ...
    if (selRegion) {
      return (regionGroups[selRegion] || []).map((c) => ({
        name:
          c.label
            .replace(new RegExp('^' + selRegion + '[-_ ]?', 'i'), '')
            .trim() || c.label,
        fullName: c.label,
    ...
    return clusters.map((c) => ({
      name:
        c.label
          .replace(new RegExp('^' + (regionCodes[0] || '') + '[-_ ]?', 'i'), '')
          .trim() || c.label,
      fullName: c.label,
      A1: pct((c.actC || {})['A1 - NORMAL'], c.total),
      A2: pct((c.actC || {})['A2 - ANOMALY'], c.total),
  ...
  const openDrill = useCallback(
    (label, color, countKey) => {
      ...
      setDrill({
        ...
        countKey,
        rows: canvList,
        total: canvList.reduce((s, r) => s + r.count, 0),
      });
    },
    [view.canvassers]
  );

  const getCanvasserRows = useCallback(
    (canvasserName, clusterLabel, drillKey) => {
      const cl =
        clusters.find((c) => c.label === clusterLabel) ||
        clusters.find((c) =>
          (c.rawRows || []).some((r) => r['Canvasser'] === canvasserName)
        );
      ...
      const ff = (r) => {
        ...
        const loc = String(
          r['Location Status'] || r['_LOC'] || ''
        ).toUpperCase();
        const inR = String(r['In Range'] || '').toLowerCase();
        const durSt = String(
          r['Duration Status'] || r['_DUR'] || ''
        ).toUpperCase();
        const disSt = String(
          r['Distance Status'] || r['_DIS'] || ''
        ).toUpperCase();
        switch (drillKey) {
          case 'A1':
            return vs === 'VALID';
          case 'A2':
            return vs === 'OBSERVE' || vs === 'INVESTIGATE';
          case 'A3':
            return vs === 'INCOMPLETE';
          case 'VALID':
            return vs === 'VALID';
          case 'OBSERVE':
            return vs === 'OBSERVE';
          case 'INVESTIGATE':
            return vs === 'INVESTIGATE';
          case 'INCOMPLETE':
            return vs === 'INCOMPLETE';
          case 'DUR_NORMAL':
            return durSt === 'NORMAL' || (!isNaN(dur) && dur >= 3 && dur <= 60);
          case 'DUR_SHORT':
            return durSt === 'SHORT' || (!isNaN(dur) && dur > 0 && dur < 3);
          case 'DUR_LONG':
            return durSt === 'LONG' || (!isNaN(dur) && dur > 60);
          case 'DIS_NEAR':
            return disSt === 'NEAR' || (dIn <= 100 && dIn > 0);
          case 'DIS_MID':
            return disSt === 'MID' || (dIn > 100 && dIn <= 1000);
          case 'DIS_FAR':
            return disSt === 'FAR' || dIn > 1000;
          case 'DIS_INC':
            return vs === 'INCOMPLETE';
          case 'LOC_MATCH':
            return loc === 'MATCH';
          case 'LOC_NOTMATCH':
            return loc === 'NOT MATCH';
          case 'LOC_INC':
            return vs === 'INCOMPLETE';
          case 'IR_YES':
            return inR === 'yes' || inR === 'y' || inR === '1';
          case 'IR_NO':
            return (
              inR === 'no' || inR === 'n' || inR === '0' || inR === 'false'
            );
          default:
            return true;
        }
      ...
      return rows.sort(
        (a, b) =>
          new Date(a['Planned Visit Date'] || 0) -
          new Date(b['Planned Visit Date'] || 0)
      );
    },
    [clusters]
  );
  ...
  const ths = (key) => ({
    ...
    fontSize: 11,
    fontWeight: 700,
    color: sk === key ? '#60a5fa' : t.muted,
    cursor: 'pointer',
    letterSpacing: '0.05em',
    whiteSpace: 'nowrap',
    userSelect: 'none',
    borderBottom: `2px solid ${sk === key ? P.accent : t.border}`,
  });
  ...
  const compLabel = selCluster
    ? `Outlet Type — ${selCluster}`
    : selRegion
    ? `Cluster dalam ${selRegion}`
    : regionCodes.length > 1
    ? `Perbandingan Region`
    : `Cluster dalam Region ${regionCodes[0] || ''}`;
    ...
    <>
      ...
      >
        ...
        <div
          style={{
            background: dark
              ? 'linear-gradient(135deg,#040e1e,#071830)'
              : t.card,
            borderBottom: `1px solid ${t.border}`,
            padding: '14px 22px',
            display: 'flex',
        ...
        >
          ...
          >
            ...
            >
              {clusters.length} cluster · {regionCodes.length} region ·{' '}
              {(national.total || 0).toLocaleString()} aktivitas
            </div>
            <button
              onClick={toggleDark}
              style={{
          ...
          >
            <button
              ...
              style={{
                ...
                background: 'transparent',
                whiteSpace: 'nowrap',
                color: !selRegion && !selCluster ? '#3b82f6' : t.muted,
                borderBottom: `3px solid ${
                  !selRegion && !selCluster ? '#3b82f6' : 'transparent'
                }`,
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    background: 'transparent',
                    whiteSpace: 'nowrap',
                    color: isActive ? rc : t.muted,
                    borderBottom: `3px solid ${isActive ? rc : 'transparent'}`,
                    transition: 'all 0.15s',
                    display: 'flex',
                    alignItems: 'center',
                    gap: 5,
                ...
                >
                  ...
                  <span
                    style={{
                      fontSize: 10,
                      background: isActive ? rc + '22' : t.cardAlt,
                      color: isActive ? rc : t.muted,
                      padding: '1px 6px',
                      borderRadius: 999,
                      fontWeight: 600,
            ...
            >
              <button
                ...
                style={{
                  ...
                  background: 'transparent',
                  whiteSpace: 'nowrap',
                  color: !selCluster ? '#60a5fa' : t.muted,
                  borderBottom: `2px solid ${
                    !selCluster ? '#60a5fa' : 'transparent'
                  }`,
                  transition: 'all 0.15s',
                }}
              >
              ...
              {(regionGroups[selRegion] || []).map((cl, i) => {
                ...
                return (
                  <button
                    key={cl.label}
                    onClick={() => setSelCluster(cl.label)}
                    style={{
                      padding: '8px 14px',
                      border: 'none',
                      cursor: 'pointer',
                      ...
                      background: 'transparent',
                      whiteSpace: 'nowrap',
                      color: isActive ? cl.color : t.muted,
                      borderBottom: `2px solid ${
                        isActive ? cl.color : 'transparent'
                      }`,
                      transition: 'all 0.15s',
                    }}
                  >
        ...
        >
          ...
          {selRegion && (
            <>
              ...
              <span
                style={{
                  cursor: 'pointer',
                  color: regionAgg[selRegion]?.color || t.muted,
                }}
                onClick={() => setSelCluster(null)}
              >
                Region {selRegion}
          ...
          {selCluster && (
            <>
              <span>›</span>
              <span style={{ color: view.color || t.text }}>{selCluster}</span>
            </>
          )}
          <span
            style={{
        ...
        <div style={{ padding: '14px 22px' }}>
          ...
          >
            {[
              {
                label: 'Total',
                val: T.toLocaleString(),
                icon: '📋',
                color: P.accent,
              },
              {
                label: 'A1 Normal',
                val: pctS(ac['A1 - NORMAL'], T),
                icon: '✅',
                color: P.a1,
                sub: (ac['A1 - NORMAL'] || 0).toLocaleString(),
                drill: () => openDrill('A1 - Normal', P.a1, 'A1'),
              },
              {
                label: 'A2 Anomaly',
                val: pctS(ac['A2 - ANOMALY'], T),
                icon: '⚠️',
                color: P.a2,
                sub: (ac['A2 - ANOMALY'] || 0).toLocaleString(),
                drill: () => openDrill('A2 - Anomaly', P.a2, 'A2'),
              },
              {
                label: 'A3 Incomplete',
                val: pctS(ac['A3 - INCOMPLETE'], T),
                icon: '🔵',
                color: P.a3,
                sub: (ac['A3 - INCOMPLETE'] || 0).toLocaleString(),
                drill: () => openDrill('A3 - Incomplete', P.a3, 'A3'),
              },
              {
                label: 'Investigate',
                val: pctS(vc['INVESTIGATE'], T),
                icon: '🔍',
                color: P.investigate,
                sub: (vc['INVESTIGATE'] || 0).toLocaleString(),
                drill: () =>
                  openDrill('Investigate', P.investigate, 'INVESTIGATE'),
              },
              {
                label: 'Canvasser',
                val: view.canvassers.length,
                icon: '👤',
                color: '#a78bfa',
              },
            ].map((k, i) => (
              <div
                key={i}
                onClick={k.drill || undefined}
                style={{
                  ...card({
                    borderTop: `3px solid ${k.color}`,
                    cursor: k.drill ? 'pointer' : 'default',
                  }),
                ...
                onMouseEnter={(e) => {
                  ...
                  if (k.drill)
                    e.currentTarget.style.boxShadow = `0 6px 20px ${k.color}30`;
                }}
                onMouseLeave={(e) => {
              ...
              >
                ...
                <div style={{ fontSize: 19, fontWeight: 800, color: k.color }}>
                  {k.val}
                </div>
                {k.sub && (
            ...
            {tabs.map((tb) => (
              <button
                ...
                style={{
                  ...
                  borderRadius: 9,
                  border: `1px solid ${
                    tab === tb.id ? 'transparent' : t.border
                  }`,
                  cursor: 'pointer',
                  fontSize: 11,
                  fontWeight: 700,
                  ...
                  background:
                    tab === tb.id
                      ? 'linear-gradient(135deg,#1d5fc0,#2d8ef5)'
                      : t.card,
                  color: tab === tb.id ? '#fff' : t.muted,
                  boxShadow:
                    tab === tb.id ? '0 4px 14px rgba(29,95,192,0.3)' : 'none',
                }}
              >
                {tb.label}
              </button>
            ...
            <div style={{ display: 'grid', gap: 16 }}>
              ...
              >
                <div style={card()}>
                  ...
                  <div
                    style={{ fontSize: 11, color: t.muted, marginBottom: 10 }}
                  >
                    {view.label}
                  </div>
                    ...
                    <PieChart>
                      <Pie
                        ...
                        onClick={(d) => {
                          ...
                          openDrill(d.name, d.color, m[d.skey]);
                        }}
                      >
                        {ACT.map((s, i) => (
                          <Cell
                  ...
                  {ACT.map((s, i) => {
                    ...
                    return (
                      <div
                        key={i}
                        onClick={() => openDrill(s.label, s.color, aKey)}
                        style={{
                          display: 'flex',
                          alignItems: 'center',
                          gap: 8,
                  ...
                  >
                    {VIS.map((s, i) => (
                      <div
                        key={i}
                        onClick={() => openDrill(s.label, s.color, s.key)}
                        style={{
                          display: 'flex',
                          alignItems: 'center',
                          gap: 6,
                ...
                <div style={card()}>
                  ...
                  {(() => {
                    const wA2 = [...view.canvassers]
                      .filter((c) => c.total >= 10)
                      .sort((a, b) => b.a2p - a.a2p)[0];
                    const wA3 = [...view.canvassers]
                      .filter((c) => c.total >= 10)
                      .sort((a, b) => b.a3p - a.a3p)[0];
                    const wInv = [...view.canvassers]
                      .filter((c) => c.total >= 10)
                      .sort((a, b) => b.invP - a.invP)[0];
                    const topDay = [...(view.trend || [])].sort(
                      (a, b) => b.total - a.total
                    )[0];
                    return [
                      {
                        icon: '✅',
                        color: P.a1,
                        title: 'A1 Normal Rate',
                        desc: `${pctS(ac['A1 - NORMAL'], T)} (${(
                          ac['A1 - NORMAL'] || 0
                        ).toLocaleString()}) aktivitas berjalan normal.`,
                      },
                      {
                        icon: '⚠️',
                        color: P.a2,
                        title: 'A2 Anomaly Tertinggi',
                        desc: wA2
                          ? `${wA2.name} [${wA2.cluster}]: ${wA2.a2p.toFixed(
                              1
                            )}% dari ${wA2.total} aktivitas`
                          : '–',
                      },
                      {
                        icon: '🔵',
                        color: P.a3,
                        title: 'A3 Incomplete Tertinggi',
                        desc: wA3
                          ? `${wA3.name} [${wA3.cluster}]: ${wA3.a3p.toFixed(
                              1
                            )}% (${wA3.A3} aktivitas)`
                          : '–',
                      },
                      ...
                      },
                      {
                        icon: '⏱',
                        color: '#f97316',
                        title: 'Durasi Singkat (SHORT)',
                        desc: `${(
                          dc['SHORT'] || 0
                        ).toLocaleString()} aktivitas durasi singkat — perlu verifikasi.`,
                      },
                      {
                        icon: '📅',
                        color: '#34d399',
                        ...
                        desc: topDay
                          ? `${
                              topDay.date
                            }: ${topDay.total.toLocaleString()} aktivitas (A1: ${pctS(
                              topDay.A1,
                              topDay.total
                            )})`
                          : '–',
              ...
              {selCluster ? (
                ...
                >
                  {[
                    {
                      label: '🏆 Top 5 A1 Normal',
                      color: P.a1,
                      key: 'A1',
                      sort: 'A1',
                    },
                    {
                      label: '⚠️ Top 5 A2 Anomaly',
                      color: P.a2,
                      key: 'A2',
                      sort: 'A2',
                    },
                    {
                      label: '🔵 Top 5 A3 Incomplete',
                      color: P.a3,
                      key: 'A3',
                      sort: 'A3',
                    },
                  ].map((cat, ci) => {
                    const top5 = [...view.canvassers]
                      .sort((a, b) => b[cat.sort] - a[cat.sort])
                      .slice(0, 5);
                      ...
                      <div key={ci} style={card()}>
                        ...
                        {top5.map((cv, i) => (
                          <div key={i} style={{ marginBottom: 10 }}>
                            ...
                            >
                              {cv.cluster} · {pctS(cv[cat.sort] || 0, cv.total)}
                            </div>
                          </div>
                        ))}
                        <button
                          onClick={() =>
                            openDrill(cat.label, cat.color, cat.key)
                          }
                          style={{
                            width: '100%',
                  ...
                  <div style={card()}>
                    ...
                    >
                      <div>
                        ...
                        >
                          {compLabel} — klik bar untuk drill-down
                        </div>
                      </div>
                      <div
                      ...
                      >
                        {compData.map((d, i) => (
                          <div
                            ...
                            onClick={() => {
                              if (selRegion) {
                                setSelCluster(d.fullName);
                              } else if (regionCodes.length > 1) {
                                setSelRegion(d.name);
                              } else {
                                setSelRegion(regionCodes[0]);
                                setSelCluster(d.fullName);
                              }
                            }}
                          >
                      ...
                      <ResponsiveContainer width="100%" height={220}>
                        <BarChart
                          ...
                          onClick={(d) => {
                            ...
                            const item = compData.find(
                              (x) => x.name === d.activeLabel
                            );
                            if (!item) return;
                            if (selRegion) {
                              setSelCluster(item.fullName);
                            } else if (regionCodes.length > 1) {
                              setSelRegion(
                                item.fullName.replace('Region ', '')
                              );
                            } else {
                              setSelRegion(regionCodes[0]);
                              setSelCluster(item.fullName);
                          ...
                          <tbody>
                            {compData.map((d, i) => (
                              ...
                              >
                                ...
                                <td
                                  style={{ padding: '7px 8px', color: t.muted }}
                                >
                                  {fmtK(d.total)}
                                </td>
                                <td
                                ...
                                <td
                                  style={{
                                    padding: '7px 8px',
                                    color: d.A2 >= 40 ? P.investigate : P.a2,
                                  }}
                                >
                                  {d.A2.toFixed(1)}%
                                </td>
                                <td style={{ padding: '7px 8px', color: P.a3 }}>
                                  {d.A3.toFixed(1)}%
                                </td>
                              </tr>
                            ))}
            ...
            <div style={{ display: 'grid', gap: 16 }}>
              <div style={card()}>
                <div style={{ fontWeight: 700, marginBottom: 4 }}>
                  Volume Aktivitas per Hari
                </div>
                <div style={{ fontSize: 11, color: t.muted, marginBottom: 14 }}>
                  Planned Visit Date · {view.label}
                </div>
                <ResponsiveContainer width="100%" height={270}>
                  <BarChart
                    data={view.trend.map((d) => ({
                      name: d.date.slice(5),
                      A1: d.A1,
                      A2: d.A2,
                      A3: d.A3,
                          ...
                          {tRev
                            ...
                            .map((d, i) => (
                              ...
                              >
                                ...
                                <td
                                  style={{ padding: '8px 12px', color: P.a1 }}
                                >
                                  {(d.A1 || 0).toLocaleString()}
                                </td>
                                <td
                                  style={{ padding: '8px 12px', color: P.a2 }}
                                >
                                  {(d.A2 || 0).toLocaleString()}
                                </td>
                                <td
                                  style={{ padding: '8px 12px', color: P.a3 }}
                                >
                                  {(d.A3 || 0).toLocaleString()}
                                </td>
                                <td
                                  style={{
                                    padding: '8px 12px',
              ...
              {['Volume', 'Persentase'].map((title, mi) => (
                <div key={mi} style={card()}>
                  <div style={{ fontWeight: 700, marginBottom: 14 }}>
                    {title} per Outlet Type
                  </div>
                  <ResponsiveContainer width="100%" height={250}>
                    <BarChart
                      data={view.outletData.map((d) =>
                        mi === 0
                          ? {
                              name: d.type.replace('RO ', ''),
                              A1: d.A1,
                    ...
                    >
                      ...
                      <YAxis
                        tick={{ fill: t.muted, fontSize: 10 }}
                        tickFormatter={mi === 0 ? fmtK : (v) => v + '%'}
                        unit={mi === 1 ? '%' : ''}
                        domain={mi === 1 ? [0, 100] : undefined}
                      />
                      <Tooltip content={mkTip} />
                      <Legend
                        formatter={(v) => (
                    ...
                    <tbody>
                      {view.outletData.map((d, i) => (
                        ...
                        >
                          ...
                          <td style={{ padding: '9px 12px', color: P.a1 }}>
                            {(d.A1 || 0).toLocaleString()}
                          </td>
                          <td style={{ padding: '9px 12px', color: P.a2 }}>
                            {(d.A2 || 0).toLocaleString()}
                          </td>
                          <td style={{ padding: '9px 12px', color: P.a3 }}>
                            {(d.A3 || 0).toLocaleString()}
                          </td>
                          <td
                            style={{
                          ...
                          >
                            {(d.INVESTIGATE || 0).toLocaleString()}
                          </td>
                          ...
                          <td
                            style={{
                              padding: '9px 12px',
                              color:
                                pct(d.A2, d.total) >= 40 ? P.investigate : P.a2,
                            }}
                          >
                            {pctS(d.A2, d.total)}
                          </td>
                    ...
                    {(view.censusData || []).map((d, i) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            {[
                              { l: 'A1 Normal', v: d.A1 || 0, c: P.a1 },
                              { l: 'A2 Anomaly', v: d.A2 || 0, c: P.a2 },
                              { l: 'A3 Incomplete', v: d.A3 || 0, c: P.a3 },
                            ].map((s, j) => (
                              <div
                                key={j}
                                style={{
              ...
              {[
                ...
                },
                {
                  ...
                  data: [
                    ...
                    { l: 'Mid', c: P.mid, v: di['MID'] || 0, dk: 'DIS_MID' },
                    { l: 'Far', c: P.far, v: di['FAR'] || 0, dk: 'DIS_FAR' },
                    {
                      l: 'Incomplete',
                      c: P.a3,
                      v: di['INCOMPLETE'] || 0,
                      ...
                      <>
                        ...
                        {sec.data.map((d, i) => (
                          <div
                            ...
                            onClick={() =>
                              d.dk &&
                              openDrill(sec.title + ' · ' + d.l, d.c, d.dk)
                            }
                            style={{
                              display: 'flex',
                              alignItems: 'center',
                            ...
                            onMouseEnter={(e) => {
                              if (d.dk) e.currentTarget.style.opacity = '0.75';
                            }}
                            onMouseLeave={(e) =>
                              (e.currentTarget.style.opacity = '1')
                            }
                          >
                            ...
                            />
                            <span
                              style={{ fontSize: 11, color: t.muted, flex: 1 }}
                            >
                              {d.l}
                            </span>
                            <span
                ...
                {(() => {
                  ...
                  return (
                    ...
                    >
                      <div>
                        <ResponsiveContainer width="100%" height={160}>
                          <PieChart>
                            <Pie
                              data={[
                                { name: 'In Range', value: irYes, color: P.a1 },
                                {
                                  name: 'Out of Range',
                                  value: irNo,
                                  color: P.investigate,
                              ...
                              onClick={(d) =>
                                openDrill(
                                  d.name,
                                  d.color,
                                  d.name === 'In Range' ? 'IR_YES' : 'IR_NO'
                                )
                              }
                            >
                              {[P.a1, P.investigate].map((col, i) => (
                                <Cell
                                  key={i}
                                  fill={col}
                                  style={{ cursor: 'pointer' }}
                      ...
                      >
                        {[
                          { l: '✓ In Range', v: irYes, c: P.a1, dk: 'IR_YES' },
                          {
                            l: '✗ Out of Range',
                            v: irNo,
                            c: P.investigate,
                          ...
                          <div
                            key={i}
                            onClick={() => openDrill(d.l, d.c, d.dk)}
                            style={{
                              display: 'flex',
                              alignItems: 'center',
                              gap: 10,
              ...
              return (
                <div>
                  ...
                  >
                    ...
                    {[
                      { id: 'all', label: 'Semua' },
                      { id: 'high_a2', label: '⚠️ A2 ≥30%' },
                      { id: 'high_a3', label: '🔵 A3 ≥20%' },
                      { id: 'top_a1', label: '✅ A1 ≥80%' },
                      { id: 'inv', label: '🔍 Inv ≥5%' },
                    ].map((f) => (
                      <button
                        key={f.id}
                  ...
                  <div style={{ ...card({ padding: 0 }), overflow: 'hidden' }}>
                    <div style={{ overflowX: 'auto' }}>
                      ...
                      >
                        ...
                        <tbody>
                          {sorted
                            .slice(cPg * CPG, (cPg + 1) * CPG)
                            .map((c, i) => (
                              <tr
                                key={c.name + c.cluster}
                                style={{
                                  borderBottom: `1px solid ${t.border}`,
                                  background:
                                    i % 2 === 0 ? 'transparent' : t.rowAlt,
                              ...
                              >
                                ...
                                <td
                                  style={{ padding: '7px 10px', color: P.a1 }}
                                >
                                  {c.A1.toLocaleString()}
                                </td>
                                <td
                                  style={{ padding: '7px 10px', color: P.a2 }}
                                >
                                  {c.A2}
                                </td>
                                <td
                                  style={{ padding: '7px 10px', color: P.a3 }}
                                >
                                  {c.A3}
                                </td>
                                <td
                                  style={{
                                    ...
                                    color:
                                      c.INVESTIGATE > 0
                                        ? P.investigate
                                        : t.muted,
                                    fontWeight: c.INVESTIGATE > 0 ? 700 : 400,
                                  }}
                                >
                                  {c.INVESTIGATE}
                                </td>
                                    ...
                                    >
                                      <div
                                        style={{
                                          ...
                                          height: '100%',
                                          borderRadius: 3,
                                          background: c.a1p >= 70 ? P.a1 : P.a2,
                                        }}
                                      />
                                    </div>
                                    <span
                                      style={{
                                        color: c.a1p >= 70 ? P.a1 : P.a2,
                                        fontWeight: 700,
                                        fontSize: 11,
                                      }}
                                    >
                                ...
                                <td
                                  style={{
                                    padding: '7px 10px',
                                    color: c.a2p >= 40 ? P.investigate : P.a2,
                                  }}
                                >
                                  {c.a2p.toFixed(1)}%
                                </td>
                                ...
                                <td
                                  style={{
                                    padding: '7px 10px',
                                    color:
                                      c.invP >= 5 ? P.investigate : t.muted,
                                    fontWeight: c.invP >= 5 ? 700 : 400,
                                  }}
                                >
                                  {c.invP.toFixed(1)}%
                                </td>
                                <td
                                  style={{
                                    padding: '7px 10px',
                                    color:
                                      c.avgDur != null && c.avgDur < 2
                                        ? P.short
                                        : t.muted,
                                  }}
                                >
                                  {c.avgDur != null
                                    ? c.avgDur.toFixed(1) + ' m'
                                    : '—'}
                                </td>
                                <td
                                  style={{
        ...
        >
          XLSMART Analytics · Klik status di chart untuk lihat breakdown
          canvasser
        </div>
      </div>
      <DrillDownPanel
        ...
        onCanvasserClick={(r) => {
          ...
          setCanvDetail({
            ...
            color: drill.color,
            rows,
            drillKey: drill.countKey,
          });
        }}
      />
      <CanvasserDetailPanel
        detail={canvDetail}
        onClose={() => setCanvDetail(null)}
        t={t}
      />
    </>
  );
